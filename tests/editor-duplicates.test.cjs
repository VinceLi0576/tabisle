...
test('EDITOR_LOAD 同时带回同域分布：同一个站散在哪几个夹、当前这份在哪',async()=>{
 const p=setup(),r=await p.call('EDITOR_LOAD');
 assert.equal(r.domain.root,'example.com');
 assert.equal(r.domain.total,3);                       // 三条都是 example.com，锚点/参数不同也算
 assert.equal(r.domain.folders.length,1);
 assert.equal(r.domain.folders[0].path,'收藏夹栏 / 资料');
 assert.equal(r.domain.folders[0].hasCurrent,true);
 assert.equal(r.domain.folders[0].items.filter(i=>i.current).length,1);
});
